...
running at least version 6.0. The C<es> configuration key may be set to the URL
...
configured or guessed from C<@INC> C<perls_dir>, which defaults to F<perls> in
the application directory. The C<install> command can be used to use
...
of one or more Perls.
  ...
  ./perldoc-browser.pl index --functions 5.10.1 # only reindex function names/content
  ./perldoc-browser.pl index --variables 5.10.1 # only reindex variable names
  ./perldoc-browser.pl index --faqs 5.10.1 # only reindex faq content
